...
def lastReleaseDate(tag):
    content = ghApiClient.readUrl('repos/swagger-api/swagger-parser/releases/tags/' + tag)
...
def addRelease(release_title, tag, content):
    ...
    content = ghApiClient.postUrl('repos/swagger-api/swagger-parser/releases', payload)
    return content
