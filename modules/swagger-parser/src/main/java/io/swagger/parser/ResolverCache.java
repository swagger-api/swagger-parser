 ...
 * expensive operations like: 1) reading a remote URL with authorization (e.g.
 * using RemoteURL.java) 2) reading the contents of a file into memory 3)
 * extracting a sub object from a json/yaml tree 4) de-serializing json strings
 * into objects
 */
public class ResolverCache {

	private static final Pattern PARAMETER_PATTERN = Pattern
			.compile("^" + RefType.PARAMETER.getInternalPrefix() + "(?<name>.+)");
	private static final Pattern DEFINITION_PATTERN = Pattern
			.compile("^" + RefType.DEFINITION.getInternalPrefix() + "(?<name>.+)");
	private static final Pattern RESPONSE_PATTERN = Pattern
			.compile("^" + RefType.RESPONSE.getInternalPrefix() + "(?<name>.+)");
	...
	/*
	 * a map that stores original external references, and their associated
	 * renamed references
	 */
	...
	private <T> T loadReference(String ref, RefFormat refFormat, Class<T> expectedType) {
		...
		// a definition path is defined, meaning we need to "dig down" through
		// the JSON tree and get the desired entity
		...
		for (String jsonPathElement : jsonPathElements) {
			...
			// if at any point we do find an element we expect, print and error
			// and abort
			if (tree == null) {
